...
class ApiTests(unittest.TestCase):
    ...
    def test_rejects_invalid_media_options(self):
        ...
        self.assertEqual(self.client.post("/", json={"url": url, "downloadMode": "exec"}).status_code, 400)
    ...
    def test_enforces_active_job_limit(self):
        ...
        self.assertEqual(
            self.client.post("/", json={"url": "https://example.com/video"}).status_code,
            429,
        )

    def test_creates_valid_job_without_starting_download(self):
        with patch.object(server, "Thread") as thread:
            response = self.client.post("/", json={"url": "https://example.com/video"})
